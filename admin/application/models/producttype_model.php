...
class Producttype_model extends CI_Model
{
    ...
    {
        $sql =" SELECT p.* , t.`name` parenttype_name  FROM  product_type p  LEFT JOIN  product_type t ON t.id = p.parenttype_id ORDER BY p.id DESC  LIMIT " . $start . "," . $limit;
    ...
    public function save_producttype()
    {
        date_default_timezone_set("Asia/Bangkok");
        $data_producttype = array(
            'name' => $this->input->post('name'),
        ...
        $this->db->insert("product_type", $data_producttype);
